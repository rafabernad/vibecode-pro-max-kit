...
const { buildPayload, parseArgs, readPlan, renderText } = require('./review-situation-scan.cjs');
...
test('task-folder convention plan is detected as primary plan', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'review-situation-task-folder-'));
  const repo = path.join(root, 'repo');
  fs.mkdirSync(repo);
  git(repo, ['init']);
  git(repo, ['config', 'user.email', 'test@example.com']);
  ...
  const taskFolderPlan = path.join(repo, 'process', 'general-plans', 'active', 'test_27-05-26', 'test_PLAN_27-05-26.md');
  writeFile(taskFolderPlan, [
    '# Test Task-Folder Plan',
    '',
    '**Status**: ACTIVE',
  ].join('\n'));
  git(repo, ['add', '.']);
  ...
  try {
    ...
    assert.ok(
      allPlanPaths.some((p) => p === 'process/general-plans/active/test_27-05-26/test_PLAN_27-05-26.md'),
