...
| Context | Skip? |
|---|---|
| General / top-level flow, non-trivial work | Default: runs. May be skipped only for an orchestrator-classified quick bounded tranche when the tranche boundary, user-facing goal, and acceptance checks are already concrete from repo evidence. |
...
> **Skip relationship:** For most non-trivial work, SPEC runs and INNOVATE is optional. In the quick bounded tranche lane, SPEC may be skipped when the first tranche is already concrete enough to lock directly in the plan. INNOVATE is skipped when the "how" is mechanical (one obvious path, no design choice). SPEC never depends on INNOVATE output — INNOVATE is downstream of SPEC when SPEC is present.
